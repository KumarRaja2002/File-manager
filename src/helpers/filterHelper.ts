...
export const filterHelper = {
  ...
  getFileTypeFilter: (query: any, fileType: string, userId: number) => {
    ...
    if (query.categoryId) {
      filters.push(sql`${files.category_id} = ${query.categoryId}`);
    }
  ...
  categoriesNameSearchFilter: (query: any) => {
    const filters: any[] = [];

    if (query.search_string) {
      filters.push(ilike(categories.name, `%${query.search_string}%`));
    }
    if (query.created_by) {
      filters.push(eq(categories.created_by, query.created_by));
    }

    filters.push(eq(categories.status, 'active'));
